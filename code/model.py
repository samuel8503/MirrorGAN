...
class RNN_ENCODER(nn.Module):
    def __init__(self, ntoken, ninput=300, drop_prob=0.5,
        ...
        self.n_steps = cfg.TEXT.WORDS_NUM - 1
    ...
    def define_module(self):
        ...
        if self.rnn_type == 'LSTM':
            ...
            self.rnn = nn.LSTM(self.ninput, self.nhidden,
                               ...
                               dropout=self.drop_prob if self.nlayers > 1 else 0,
...
class EncoderCNN(nn.Module):
    def __init__(self, embed_size):
        ...
        if cfg.TRAIN.FLAG:
            ...
            self.embed_size = 256  # define a uniform ranker
    ...
    def forward(self, images):
        ...
        features = self.bn(self.linear(features)) # batchsize
...
class DecoderRNN(nn.Module):
    ...
    def forward(self, features, captions, lengths):
        ...
        embeddings = torch.cat((features.unsqueeze(1), embeddings.squeeze(2)), 1)
        packed = pack_padded_sequence(embeddings, lengths, batch_first=True, enforce_sorted=False) 
        hiddens, _ = self.lstm(packed)
        outputs = self.linear(hiddens[0])
        return outputs
    
    def sample(self, features, states=None): #TODO: 
        ...
        for i in range(self.max_seg_length):
            ...
            #sampled_ids.append(self.softmax(outputs))
            sampled_ids.append(predicted)
            inputs = self.embed(predicted)                       # inputs: (batch_size, embed_size)
            inputs = inputs.unsqueeze(1)                         # inputs: (batch_size, 1, embed_size)
        sampled_ids = torch.stack(sampled_ids, 1)                # sampled_ids: (max_seq_length, batch_size, vocab_size)
        return sampled_ids
...
class NEXT_STAGE_G(nn.Module):
    ...
    def forward(self, h_code, c_code, word_embs, sent_emb, mask):
        """
            h_code1(query):  batch x idf x ih x iw (queryL=ihxiw)
            word_embs(context): batch x cdf x sourceL (sourceL=seq_len)
            sent_emb(context): batch x cdf
            c_code1: batch x idf x queryL
            att1: batch x sourceL x queryL
        """
        self.w_att.applyMask(mask)
        c_code1, w_att = self.w_att(h_code, word_embs)
        c_code2, s_att = self.s_att(h_code, sent_emb)
        h_c_code = torch.cat((h_code, c_code1, c_code2), 1)
        out_code = self.residual(h_c_code)

        # state size ngf/2 x 2in_size x 2in_size
        out_code = self.upsample(out_code)

        return out_code, w_att, s_att
...
class G_NET(nn.Module):
    ...
    def forward(self, z_code, sent_emb, word_embs, mask):
        ...
        if cfg.TREE.BRANCH_NUM > 1:
            h_code2, w_att1, s_att1 = \
                self.h_net2(h_code1, c_code, word_embs, sent_emb, mask)
        ...
        if cfg.TREE.BRANCH_NUM > 2:
            h_code3, w_att2, s_att2 = \
                self.h_net3(h_code2, c_code, word_embs, sent_emb, mask)
        ...
        return fake_imgs, w_att_maps, mu, logvar, s_att_maps
